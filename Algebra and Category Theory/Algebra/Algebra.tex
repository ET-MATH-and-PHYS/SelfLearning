...
\begin{defn}
    Define the $n$th \Emph{cyclotomic polynomial} $\Psi_n(x)$ to be the polynomial whose roots are the primitive $n$th roots of unity: \begin{equation*}
...
\begin{defn}
    A polynomial $f(x) \in F[x]$ can be \Emph{solved by radicals} if all of its roots can be solved for in terms of radicals.
\end{defn}
...
\end{thm}
\begin{proof}
    Suppose first that $f(x)$ can be solved by radicals. Then each root of $f(x)$ is contained in a Galois root extension over $F$. The composite $L$ of such extensions is again Galois, and by the work in the proof, a root extension. Let $G_i$ be the subgroups corresponding to the subfields $K_i, i = 0,1,...,s-1$. Since $\text{Gal}(K_{i+1}/K_i)= G_i/G_{i+1}$ it follows that the Galois group $G = \text{Gal}(L/F)$ (To be continued)
\end{proof}
