...
\begin{proposition}
    Let $R$ be a ring. Then the function $r\mapsto \lambda_r$ is an injective ring homomorphism $$\lambda:R\rightarrow \en(R)$$
\end{proposition}
...
\begin{definition}
    For a ring $R$, $a \in R$ is called a (left-)\Emph{zero divisor} if there exists $b \in R$, $b \neq 0$, such that $ab = 0$.
\end{definition}
...
\begin{proposition}
    Let $R$ be a finite commutative ring. Then $R$ is an integral domain if and only if it is a field.
\end{proposition}

This follows from the fact that any injection from a finite set to itself must necessarily be a surjection.
...
We also consider a subring $S$ of a ring $R$ to be a ring whose underlying set is a subset of $R$ and such that the natural inclusion $S\hookrightarrow R$ is a ring homomorphism.
...
\begin{proposition}
    For a ring homomorphism $\varphi:R\rightarrow S$, the following are equivalent: \begin{itemize}
...
\begin{proposition}
    The ring $\Z$ is initial in the category of unital rings.
\end{proposition}


It also turns out that the inclusion $\iota:\Z\hookrightarrow \Q$ is an epimorphism which is not a surjection.
...
If $R_1$ and $R_2$ are rings, then $R_1\times R_2$ has a natural ring structure given by applying operations component-wise.
